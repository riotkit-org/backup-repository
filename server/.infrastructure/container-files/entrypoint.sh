...
exec /usr/bin/entrypoint.sh su www-data -s /bin/bash -c 'multirun -v "nginx" "php-fpm -F -O" "tail -f ${appLogPath}" "tail -f /var/log/nginx/error.log" "tail -f /var/log/nginx/access.log"'
