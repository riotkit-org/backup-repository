...
class CollectionTest(BaseTestCase):
    def test_cannot_upload_if_uploader_role_not_present(self):
        ...
        response = self.post("/api/stable/repository/collection/iwa-ait/version", "something", auth=f'Bearer {token}')
    ...
    def test_gpg_detection(self):
        ...
        response = self.post("/api/stable/repository/collection/iwa-ait/version", "hello this is not a gpg data", auth=f'Bearer {token}')
